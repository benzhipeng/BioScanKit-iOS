...
private struct RecognitionFailureView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Text(BioScanCaptureL10n.string(failure.title))
                .font(.title2.weight(.bold))
            Text(BioScanCaptureL10n.string(failure.message))
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
            Button(BioScanCaptureL10n.string("Try Again"), action: onRetry)
                .buttonStyle(.borderedProminent)
                .tint(theme.accent)
            Button(BioScanCaptureL10n.string("Back to Camera"), action: onCancel)
        }
        .padding(28)
    }
}
